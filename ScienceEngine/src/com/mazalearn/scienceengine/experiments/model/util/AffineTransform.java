...
public class AffineTransform implements Cloneable, Serializable {
   ...
   */
  public AffineTransform(float m00, float m10, float m01, float m11, float m02,
      float m12) {
   ...
   * to the matrix starts with <code>matrix[0]</code> giving the top-left entry
   * of the matrix and proceeds with the usual left-to-right and top-down
   * ordering.
   * <p>
   * If the array has only four entries, then the two entries of the last row of
   * the transformation matrix default to zero.
   * 
   * @param matrix
   *          the array of four or six floats giving the values of the matrix.
  ...
  /**
   * Instantiates a new affine transform by reading the values of the
   * transformation matrix from an array of doubles. The mapping from the array
   * to the matrix starts with <code>matrix[0]</code> giving the top-left entry
   * of the matrix and proceeds with the usual left-to-right and top-down
   * ordering.
   * <p>
   ...
   * 
   * @param matrix
   *          the array of four or six doubles giving the values of the matrix.
   * @throws ArrayIndexOutOfBoundsException
   *           if the size of the array is 0, 1, 2, 3, or 5.
   */
   ...
   * transformation are given by (m00, m01) and (m10, m11), and the translation
   * vector is (m02, m12).
   * <p>
   ...
   * TYPE_QUADRANT_ROTATION - The new basis is a rotation of the original by 90,
   * 180, 270, or 360 degrees<br/>
   * TYPE_GENERAL_ROTATION - The new basis is a rotation of the original by an
   * arbitrary angle<br/>
   * TYPE_GENERAL_TRANSFORM - The transformation can't be inverted.<br/>
   * <p>
   * Note that multiple types are possible, thus the types can be combined using
  ...
  /**
   * Gets the scale x entry of the transformation matrix (the upper left matrix
   * entry).
   * 
  ...
  /**
   * Gets the scale y entry of the transformation matrix (the lower right entry
  ...
  /**
   * Gets the shear x entry of the transformation matrix (the upper right entry
   * of the linear transformation).
   * 
  ...
  /**
   * Sets the transformation to a translation alone. Sets the linear part of the
   * transformation to identity and the translation vector to the values sent as
   * parameters. Sets the type to <code>TYPE_IDENTITY</code> if the resulting
   * AffineTransformation is the identity transformation, otherwise sets it to
  ...
  /**
   * Sets the transformation to being a rotation followed by a translation. Sets
   * the type to <code>TYPE_UNKNOWN</code>.
   * 
   * @param angle
   *          the angle of rotation in radians.
   * @param px
   ...
   * 
   * @param t
   *          the AffineTransform that this AffineTransform is multiplied by.
   */
   ...
   *           if this AffineTransform cannot be inverted (the determinant of
   *           the linear transformation part is zero).
   */
  public AffineTransform createInverse() {
    double det = getDeterminant();
    if (Math.abs(det) < ZERO) {
      throw new RuntimeException("Could not invert - determinant is 0");
    }
   ...
   *          the original point.
   * @param dst
   *          Vector2 object to be filled with the destination coordinates
   *          (where the original point is sent by this AffineTransform). May be
   *          null.
   * @return the point in the AffineTransform's image space where the original
   *         point is sent.
   */
  public Vector2 transform(Vector2 src, Vector2 dst) {
    if (dst == null) {
      dst = new Vector2();
    }
    ...
    dst.set((float) (x * m00 + y * m01 + m02), (float) (x * m10 + y * m11 + m12));
    return dst;
  }
   ...
   *          transformed.
   * @param dst
   *          the point array where the images of the points (after applying the
   *          AffineTransformation) should be placed.
   * @param dstOff
   *          the offset in the destination array where the new values should be
   *          written.
   * @param length
   ...
   */
  public void transform(Vector2[] src, int srcOff, Vector2[] dst, int dstOff,
      int length) {
    while (--length >= 0) {
      Vector2 srcPoint = src[srcOff++];
      double x = srcPoint.x;
      double y = srcPoint.y;
      Vector2 dstPoint = dst[dstOff];
      if (dstPoint == null) {
        dstPoint = new Vector2();
      }
      dstPoint.set((float) (x * m00 + y * m01 + m02), (float) (x * m10 + y * m11 + m12));
      dst[dstOff++] = dstPoint;
    }
  }

  /**
   * Applies this AffineTransform to a set of points given as an array of double
   * values where every two values in the array give the coordinates of a point;
   * the even-indexed values giving the x coordinates and the odd-indexed values
   ...
   */
  public void transform(double[] src, int srcOff, double[] dst, int dstOff,
      int length) {
    int step = 2;
  ...
  /**
   * Applies this AffineTransform to a set of points given as an array of float
   * values where every two values in the array give the coordinates of a point;
   * the even-indexed values giving the x coordinates and the odd-indexed values
   * giving the y coordinates.
   * 
   ...
   * values where every two values in the array give the coordinates of a point;
   * the even-indexed values giving the x coordinates and the odd-indexed values
   * giving the y coordinates. The destination coordinates are given as values
   ...
   *           <code>dstOff + length*2 > dst.length</code>.
   */
  public void transform(float[] src, int srcOff, double[] dst, int dstOff,
      int length) {
    while (--length >= 0) {
      float x = src[srcOff++];
      float y = src[srcOff++];
   ...
   *           <code>dstOff + length*2 > dst.length</code>.
   */
  public void transform(double[] src, int srcOff, float[] dst, int dstOff,
      int length) {
    while (--length >= 0) {
      double x = src[srcOff++];
      double y = src[srcOff++];
   ...
   *         the original point.
   */
  public Vector2 deltaTransform(Vector2 src, Vector2 dst) {
    if (dst == null) {
      dst = new Vector2();
    }
    ...
    dst.set((float) (x * m00 + y * m01), (float) (x * m10 + y * m11));
    return dst;
  }

  /**
   * Applies the linear transformation part of this AffineTransform (ignoring
   * the translation part) to a set of points given as an array of double values
   * where every two values in the array give the coordinates of a point; the
   * even-indexed values giving the x coordinates and the odd-indexed values
   * giving the y coordinates.
   * 
   * @param src
   *          the array of points to be transformed.
   ...
   * @param dst
   *          the point array where the images of the points (after applying the
   *          delta transformation) should be placed.
   * @param dstOff
   *          the offset in the destination array where the new values should be
   *          written.
   * @param length
   ...
   *           <code>dstOff + length*2 > dst.length</code>.
   */
  public void deltaTransform(double[] src, int srcOff, double[] dst,
      int dstOff, int length) {
    while (--length >= 0) {
      double x = src[srcOff++];
      double y = src[srcOff++];
   ...
   *           the linear transformation part is zero).
   */
  public Vector2 inverseTransform(Vector2 src, Vector2 dst) {
    double det = getDeterminant();
    if (Math.abs(det) < ZERO) {
      // awt.204=Determinant is zero
      throw new RuntimeException("Cannot invert. Determinant is 0");
    }
  ...
  /**
   * Applies the inverse of this AffineTransform to a set of points given as an
   * array of double values where every two values in the array give the
   ...
   * @param dst
   *          the point array where the images of the points (after applying the
   *          inverse of the AffineTransformation) should be placed.
   * @param dstOff
   *          the offset in the destination array where the new values should be
   *          written.
   * @param length
   ...
   *           the linear transformation part is zero).
   */
  public void inverseTransform(double[] src, int srcOff, double[] dst,
      int dstOff, int length) {
    double det = getDeterminant();
    if (Math.abs(det) < ZERO) {
      // awt.204=Determinant is zero
  ...
  /**
   * Creates a new shape whose data is given by applying this AffineTransform to
   * the specified shape.
   * 
   * @param src
   ...
   *          - the input stream.
   * @throws IOException
   *           - if there are I/O errors while reading from the input stream.
