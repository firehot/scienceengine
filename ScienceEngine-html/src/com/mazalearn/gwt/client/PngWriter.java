 ...
 * Terms of Use: You may use the PNG encoder free of charge for any purpose you
 * desire, as long as you do not claim credit for the original sources and agree
 * not to hold me responsible for any damage arising out of its use.<br>
...
public class PngWriter {
  ...
  static byte[] write(Pixmap pixmap) throws IOException {
    ...
    byte[] header = PngWriter.createHeaderChunk(pixmap.getWidth(),
        pixmap.getHeight());
    byte[] data = PngWriter.createDataChunk(pixmap);
    byte[] trailer = PngWriter.createTrailerChunk();
  ...
  static private byte[] createHeaderChunk(int width, int height)
      throws IOException {
   ...
   * by the PNG specification. This file comes with a minimal ZLIB encoder which
   * uses uncompressed deflate blocks (fast, short, easy, but no compression).
